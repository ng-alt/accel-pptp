...
static pid_t pppfork=0;                   /* so we can kill it after disconnect */
...
static u_int32_t call_id_pair=0;	/* call id (to terminate call) */
int window=10;
...
static void launch_pppd(char **pppaddrs, struct in_addr *inetaddrs)
{
	...
	sprintf(tmp,"%s:",inet_ntoa(inetaddrs[0]));
	sprintf(tmp+strlen(tmp),"%s",inet_ntoa(inetaddrs[1]));
